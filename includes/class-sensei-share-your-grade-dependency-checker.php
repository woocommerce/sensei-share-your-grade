...
/**
 * Sensei Share Your Grade Extension Dependencies Check
 *
 * @since 2.0.0
 */
class Sensei_Share_Your_Grade_Dependency_Checker {
	...
	public static function add_php_notice() {
		...
		printf(
			...
			esc_html__( 'Learn more about updating PHP', 'sensei-share-your-grade' ),
			/* translators: accessibility text */
			esc_html__( '(opens in a new tab)', 'sensei-share-your-grade' )
		);
		echo '</p></div>';
	}
	...
	public static function add_sensei_notice() {
		...
		$message = sprintf( __( '<strong>Sensei Share Your Grade</strong> requires that the plugin <strong>Sensei</strong> (minimum version: <strong>%1$s</strong>) is installed and activated.', 'sensei-share-your-grade' ), self::MINIMUM_SENSEI_VERSION );
		echo '<div class="error"><p>';
		echo wp_kses( $message, array( 'strong' => array() ) );
		echo '</p></div>';
	}
